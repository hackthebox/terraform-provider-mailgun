...
func deleteTemplate(t *testing.T, client *mailgun.Client, name string) *resource.DeleteResponse {
	t.Helper()
	...
	(&templateResource{client: client}).Delete(context.Background(), resource.DeleteRequest{State: state}, resp)
	return resp
}
...
func TestDelete_GenuineNotFoundIsIgnored(t *testing.T) {
	...
	resp := deleteTemplate(t, client, "gone-template")
...
func TestDelete_500MentioningNotFoundRaisesDiagnostic(t *testing.T) {
	...
	resp := deleteTemplate(t, client, "my-template")
